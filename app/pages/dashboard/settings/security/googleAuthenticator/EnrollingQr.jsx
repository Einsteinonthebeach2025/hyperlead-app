...
const EnrollingQr = ({ enrolling, qrCode, code, setCode, loading, setFactor, setQrCode, setEnrolling, setLoading, factor }) => {
  ...
  const verifyEnrollment = async (e) => {
    ...
    try {
      ...
      console.log("[2FA] Challenge response:", { challengeData, challengeError });
      ...
      console.log("[2FA] Verification response:", { verifyError });
      ...
      if (verifyError) {
        ...
        // Reload factors to confirm verified status
        const { data } = await supabase.auth.mfa.listFactors();
        const verifiedTotp = data?.all?.find(
          (f) => f.factor_type === "totp" && f.status === "verified"
        );
        setFactor(verifiedTotp || null);
        console.log("[2FA] Reloaded factors after verification:", data);
      }
    } catch (e) {
